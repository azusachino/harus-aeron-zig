...
- Current counters/interop contract: channel-status counters now use upstream-style channel-key metadata plus registration ids, `ON_SUBSCRIPTION_READY` carries a real receive-channel-status counter id, and `deploy/InteropSmoke.java` validates the core counters surface through Java `countersReader()`/`channelStatusId()`.

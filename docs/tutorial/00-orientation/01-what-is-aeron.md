...
```mermaid
graph LR
    A["Publisher Process<br/>offer(msg)"] -->|mmap<br/>shared memory| B["Media Driver<br/>Conductor<br/>Sender<br/>Receiver"]
    B -->|UDP| C["Subscriber Process<br/>poll(handler)"]
    C -->|UDP| B
```

...
## Error Handling: Error Unions and `errdefer`

```mermaid
flowchart TD
    A["Function call"] --> B{"Returns error?"}
    B -->|Yes| C["Error flows up<br/>OR caught by catch"]
    B -->|No| D["Success path<br/>errdefer not invoked"]
    C --> E["errdefer block runs<br/>cleanup happens"]
    D --> F["Code continues"]
```

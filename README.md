...
- **Language**: Zig 0.16.x (pinned via `flake.lock`)
- **Dev tooling**: Nix devShell (`nix develop`)
- **Task runner**: `make`
- **Interop pin**: Aeron `1.50.x`
...
- Override if needed: `make setup-upstream-aeron AERON_UPSTREAM_REF=1.50.x`.
- Preferred source of truth for Zig 0.16.x API/source checks: local `vendor/zig` when present.
...
- Default upstream ref is `0.16.x`, cloned into `vendor/zig`.
- Interop jars and upstream test/doc references in this repo are pinned to Aeron `1.50.x`.
